...
- [X] T006 [P] Create `backend/.env.example` with QDRANT_URL, QDRANT_API_KEY, TRANSFORMERS_CACHE, MODEL_NAME, ALLOWED_ORIGINS, RATE_LIMIT_PER_MINUTE, LOG_LEVEL placeholders
...
- [] T009 [P] Install backend dependencies in `backend/requirements.txt`: FastAPI, uvicorn, sentence-transformers, openai-agents, python-dotenv, pydantic
...
- [C] T012 Set up Neon PostgreSQL free tier account and create database `textbook_db` - See `backend/CLOUD_SETUP.md` (REMOVED - Neon is no longer used)
- [C] T013 Create Neon database schema in `backend/scripts/setup_db.py`: tables chunk_metadata, chat_queries, chat_responses with indexes (REMOVED - Neon is no longer used)
- [C] T014 Run `backend/scripts/setup_db.py` to initialize Neon database schema (REMOVED - Neon is no longer used)
- [X] T015 [P] Create backend config module in `backend/app/config.py` to load environment variables (Qdrant URL/API key, model name, CORS origins, rate limit)
...
- [C] T020 [P] Create Neon service client in `backend/app/services/neon_service.py` with connection pooling, insert_chunk_metadata(), get_chunks_by_ids() methods (REMOVED - Neon is no longer used)
...
- [X] T023 [P] Create health check endpoint `GET /health` in `backend/app/api/health.py` that checks Qdrant and embedding model status
- [X] T024 Test backend startup: `cd backend && uvicorn app.main:app --reload` and verify health endpoint at http://localhost:8000/health returns 200 (requires .env setup)
...
- [X] T043 [P] [US2] Create indexing script in `backend/scripts/index_chapters.py` that reads all website/docs/chapter-*.md files, chunks content, generates embeddings, stores in Qdrant
- [ ] T044 [US2] Run indexing script to populate Qdrant with all 6 chapters: `cd backend && python scripts/index_chapters.py` (requires cloud setup from T011)
- [ ] T045 [US2] Verify indexing completed successfully by checking Qdrant collection has ~90 vectors
- [X] T046 [P] [US2] Create RAG service in `backend/app/services/rag_service.py` with process_query() method that generates query embedding, searches Qdrant top-k=5, constructs answer from top 3 chunks
...
**Phase 2 (Foundational)**: Tasks T015, T016, T017, T018, T019, T021, T023 can run in parallel (7 concurrent tasks) after T011 complete
